...
class XPrinterManager {
  ...
  Future<void> setupXPrinter(
      {String currentPrinterIp = '192.168.123.100',
      required String newPrinterIP}) async {
    // final stream = PortScanner.discover(printerSubnet, _port, timeout: _timeout);

    // final Completer<PrinterDevice?> completer = Completer();

    final xprinter = await PortScanner.discoverFromAddress(
        currentPrinterIp, _port,
        timeout: _timeout);
